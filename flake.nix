{
  description = "Kitten Space Agency (KSA) - Linux";
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      # by the game's swapchain creation code.
      vkFixSwapchain = pkgs.stdenv.mkDerivation {
        pname = "vk-fix-swapchain";
        version = "0.1.0";
        src = ./.;

        nativeBuildInputs = [ pkgs.vulkan-headers ];
      ...
      glfwShim = pkgs.stdenv.mkDerivation {
        pname = "ksa-glfw-shim";
        version = "0.1.0";
        src = ./.;
        ...
        gameSrc = pkgs.fetchurl {
          url = "https://ksa-linux.ahwoo.com/download?file=setup_ksa_v2026.2.18.3622.tar";
          hash = "sha256-ffMHPXQnAFGWpoHbyWqEVwToabsIn88BO+1/bAFY6wI=";
        };

        buildPhase = ''
          # Extract the bundled GLFW from the game tarball
          tar xf $gameSrc linux-x64/libglfw.so.3.5
      ...
      # Libraries that are dlopen'd at runtime by the .NET runtime, GLFW, and fmod
      runtimeDeps = [
        pkgs.icu
        pkgs.libx11
        pkgs.libxcursor
      ...
      ksa = pkgs.stdenv.mkDerivation rec {
        pname = "ksa";
...
WRAPPER
      ...
      # Separate package WITHOUT the GLFW shim — uses the original bundled GLFW
      # This is needed for clean A/B testing: shim vs no-shim
      ksaNoShim = pkgs.stdenv.mkDerivation rec {
        pname = "ksa-no-shim";
        version = "2026.2.18.3622";
        ...
        installPhase = ''
          ...
          # syncshaders only (original GLFW, no shim)
          cat > $out/bin/ksa-syncshaders-only <<'WRAPPER'
#!/usr/bin/env bash
export RADV_DEBUG=syncshaders
logdir="$HOME/Documents/My Games/Kitten Space Agency/logs"
...
ts=$(date +%Y%m%d-%H%M%S)
logfile="$logdir/ksa-syncshaders-only-$ts.log"
echo "KSA [syncshaders-only, no shim] log: $logfile"
exec @out@/bin/ksa-unwrapped "$@" >"$logfile" 2>&1
WRAPPER
          substituteInPlace $out/bin/ksa-syncshaders-only --replace-fail "@out@" "$out"
          chmod +x $out/bin/ksa-syncshaders-only

          # Baseline: no shim, no syncshaders
          cat > $out/bin/ksa-baseline <<'WRAPPER'
#!/usr/bin/env bash
logdir="$HOME/Documents/My Games/Kitten Space Agency/logs"
mkdir -p "$logdir"
ts=$(date +%Y%m%d-%H%M%S)
logfile="$logdir/ksa-baseline-$ts.log"
echo "KSA [baseline, no shim, no syncshaders] log: $logfile"
exec @out@/bin/ksa-unwrapped "$@" >"$logfile" 2>&1
WRAPPER
          substituteInPlace $out/bin/ksa-baseline --replace-fail "@out@" "$out"
          chmod +x $out/bin/ksa-baseline
        ...
        meta = with pkgs.lib; {
          description = "Kitten Space Agency (no GLFW shim, for testing)";
          platforms = [ "x86_64-linux" ];
        };
      };
    in
    {
      packages.${system} = {
        inherit ksa ksaNoShim;
        default = ksa;
      };

      # nix run .#              → shim + syncshaders (default, most stable)
      # nix run .#shim-only     → shim, no syncshaders (test: does shim alone prevent crashes?)
      # nix run .#syncshaders   → syncshaders, no shim (test: does syncshaders alone suffice?)
      # nix run .#baseline      → no shim, no syncshaders (control: expected to crash)
      apps.${system} = {
        default = {
          type = "app";
          program = "${ksa}/bin/ksa";
        ...
        shim-only = {
          type = "app";
          program = "${ksa}/bin/ksa-shim-only";
        };
        syncshaders = {
          type = "app";
          program = "${ksaNoShim}/bin/ksa-syncshaders-only";
        };
        baseline = {
          type = "app";
          program = "${ksaNoShim}/bin/ksa-baseline";
        };
      };
    };
}
